#> asset:artifact/3104.auto_torch_ver3/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/3104.auto_torch_ver3/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 3104
# 神器のベースアイテム
...
# 消費アイテム ({Item: TextComponent, Count: int, Extra?: TextComponent}) (オプション)
    ...
    data modify storage asset:artifact ConsumeItem.Extra set value '{"text":"30%"}'
# 使用回数 (int) (オプション)
...
# カスタムNBT (NBTCompound) 追加で指定したいNBT (オプション)
    data modify storage asset:artifact CustomNBT set value {CustomModelData: 933}
