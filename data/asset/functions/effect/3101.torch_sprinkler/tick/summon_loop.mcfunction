...
# Motionを設定
    # 初期化
        data modify storage asset:effect Motion set value [0.0, 0.0, 0.0]
    # マクロを用いてField.Powerに応じて取得
        function asset:effect/3101.torch_sprinkler/tick/set_motion.m with storage asset:context this
    # 代入
        data modify entity @e[type=falling_block,distance=..1,tag=3101.Init,limit=1] Motion set from storage asset:effect Motion
    # ダメージを与えて描画を促す
...
# reset
    data remove storage asset:effect Motion
